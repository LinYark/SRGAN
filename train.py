...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(
        dataset=train_set, num_workers=4, batch_size=16, shuffle=True
    )
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        torch.save(
            netG.state_dict(), "epochs/netG_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
        )
        torch.save(
            netD.state_dict(), "epochs/netD_epoch_%d_%d.pth" % (UPSCALE_FACTOR, epoch)
        )
